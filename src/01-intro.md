...
**1. AIを「運任せ」ではなく「安定したパートナー」として使える**
- AIの5つの特性（暴走・手抜き・忘れっぽさ等）を理解
- 事前のガードレール（設計書）と事後のガードレール（テスト）で品質を安定化
- 再現性のある開発プロセスを習得

**2. 5-STEPワークフローで開発効率を最大化**
- 要件定義 → 設計 → タスク分解 → 実装 → 品質改善
- 各ステップで「何をAIに任せ、何を人間が判断するか」が明確に
- 研究データに基づく効果：生産性25-50%向上、品質40%向上

**3. 明日から実践できる具体的なプロンプトとツール**
- Claude Codeの効果的な使い方
- コピペで使えるプロンプト例を多数紹介
- TDD、レビュー、リファクタリングの自動化
...
**失敗例の原因を特性で説明できる**
- バグ41%増加（Uplevel）→ **手抜き**: テストを通すだけの仮実装
- コード重複8倍（GitClear）→ **暴走**: 既存コードを無視して新規追加
- レビュー負担91%増加 → **忘れっぽさ**: 指示を忘れて一貫性のないコード

**特性を理解すれば対策を打てる** → 次のスライドで5つの特性と対策を解説
...
- レーガン大統領がゴルバチョフとの交渉で繰り返し引用
- 相手を信頼しながらも、合意内容は必ず検証する姿勢
...
**AIは外部メモリが必要**
- **コンテキスト** = AIが参照できる情報（設計書、コード、会話履歴など）
- **トークン** = AIが処理するテキストの単位（日本語約1.5文字 = 1トークン）
- コンテキストには上限があり、使いすぎると思考リソースが減る
- **設計書あり**：情報密度が高く効率的 → 十分な思考リソースを確保
...
**なぜ段階的に進めるのか**
- **compact** = 会話が長くなると古い情報を切り捨てる仕組み
- **1つのサブタスク内でcompactを発生させないことが最重要**
- compactが発生すると設計意図や指示が失われ、品質が大幅低下
- 10分ルール: 10分で完了できるサイズならcompactが発生しない
- **サブタスク完了後は`/clear`でコンテキストをリセット**

**具体例：ユーザー認証機能**

❌ 悪い例（大きすぎる）:「ユーザー認証機能を実装して」→ compactが発生し品質低下

✅ 良い例（段階的）:
1. Userモデルを作成（10分）→ `/clear`
2. パスワードハッシュ化を実装（10分）→ `/clear`
3. ログインエンドポイントを作成（10分）→ `/clear`
4. JWTトークン発行を実装（10分）

**効果**: サブタスク中はcompactなし、タスク間は/clearで汚染防止
...
**Accept Edits（自動承認モード）**
- Shift+Tabで有効化
- 編集を自動承認、操作は見えて介入可能
- 日常的に使う効率化モード

**--dangerously-skip-permissions（完全自動実行）**
- CLIオプションで起動時に指定
- すべての許可を完全スキップ
- CI/コンテナ向け、俗称「YOLOモード」
...
**覚えておくこと（続き）**
- **サブタスク完了後は必ず`/clear`を実行**
  - 次のタスクでcompactが発生しにくくなる
  - 前のタスクの情報が混入しない
- AIに質問させる（暴走・手抜き・忘れっぽさを防ぐ）

**成功の条件**
- 正しく使えば生産性25-50%向上、品質40%向上（Harvard/BCG研究）
- 間違えるとコード重複8倍、レビュー負担91%増加（GitClear研究）
