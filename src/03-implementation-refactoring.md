...
### AIコードのランダム性（ガチャ）とリンターによる強制

- **AIのコードにはランダム性がある**
  - 依頼ごとに別のエンジニアに指示しているようなもの
  - コーディングスタイルが毎回異なる
  - プロジェクト全体で一貫性が失われやすい
- **対処法1: 気にしない**
  - 動けばOKという割り切り
  - 運用保守で多少の違いは許容
- **対処法2: リンター・フォーマッターで強制**
  - プロンプトで統一を依頼するより効率的
  - ruff、black、prettier等で自動整形
  - **ルールを書いてAIに従わせる**が建設的
  - CI/CDで強制すれば人間もAIも同じルールに従う
...
### Plan - 実装方針の確認（ハルシネーション防止の要）
- **なぜPlanフェーズを追加するのか**
  - 一般TDDにはない、AI駆動開発独自のフェーズ
  - **事前に情報を読ませることで存在しない関数の呼び出しを防ぐ**
- **ライブラリ・ドキュメントの事前読み込み**
  - 「このライブラリのドキュメントをWEB検索して読んで」
  - docs/に保存した仕様書を参照させる
  - AIはWEB検索と文書参照を促される
- **Issueの内容確認**
  - BDDテストシナリオ、参照設計書、実装の制約
- **AIに質問させる**
...
- **Silent Fail（静かな失敗）**
  - エラーを握りつぶして何事もなかったように続行
  - 例: `except: pass`、エラーログなしでreturn None
- **Silent Fallback（静かなフォールバック）**
  - 失敗時にデフォルト値を返して本来の処理をスキップ
  - 例: API失敗時に空リストを返す（エラー通知なし）
- **Mock Abuse（モックの乱用）**
  - テストでモックを多用し、実際の動作を検証していない
  - 例: DBアクセスを全てモック、実際のSQLが動くか不明
- **AI Self Reviewで検出させる**
  - 「Silent Fail、Silent Fallback、Mock Abuseがないか確認して」
  - AIは自分の失敗パターンを認識して修正できる
- **将来**: これらを自動検出する静的解析ツールの登場を予想
...
- **対応するAI特性**
  - ②手抜き → 完成度評価で80点以上を目指す
  - ③忘れっぽさ → /clearでコンテキストリセット、compact回避
- **学習内容**
  - 8つの質問と指示（リファクタリング、ドキュメント、知見保存）
  - 完成度0-100点評価の実践
  - **サブタスク完了後の/clearによるcompact回避**
  - 後方互換性の判断基準
- **ゴール**
  - 8つの質問と指示を実行できる
  - /clearの習慣を身につける
...
- **サブタスク完了後は必ず`/clear`でコンテキストをクリア**
  - 次のサブタスクでcompactが発生しにくくなる
  - 前のタスクの情報が次に混入しない
  - **これが品質維持の鍵**
...
### ⑧実装で得られた知見のドキュメント化
- **なぜ知見を保存するのか - 無限修正ループの防止**
  - セッション切り替え・compactで情報が失われる
  - **AIは同じ間違いを何度も繰り返す**
  - 技術的選択の経緯を残さないと無限に修正ループにハマる
- **保存すべき内容**
  - 技術的な発見・落とし穴
  - 「なぜこの方法を選んだか」の経緯
  - 試行錯誤で得た知見
- **Claude Codeへの指示**
  - 「この実装で得られた知見をdocs/lessons-learned/に保存して」
- **AIにとってコストはたいしたことない**
  - ドキュメント生成はAIの得意分野
  - 振り返りが容易になる価値は大きい
...
- **開発が進んでから気づくリファクタ要素**
  - 複数箇所でほぼ同じ処理をしているコード
  - デザインパターン適用で条件分岐を減らせる箇所
  - 共通化できるユーティリティ関数
- **まとまった開発タスク完了時にリファクタ**
  - 実装中は気づかない改善点が見える
  - 全体像が見えてから最適化できる
- **AIはリファクタリングに非常に詳しい**
  - 「どんなリファクタリングが適用可能か教えて」
  - 「このコードをより良くする方法を提案して」
  - AIが具体的なリファクタパターンを提案してくれる
- **聞きさえすれば回答してくれる**
  - AIの知識を活用しないのはもったいない
