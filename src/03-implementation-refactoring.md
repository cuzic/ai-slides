...
- **対応するAI特性**
  ...
  - ②手抜き → AI Self Reviewで完成度100点まで繰り返す
...
### 問題：AI Self Reviewで検出すべき主要な問題は？

A. 構文エラーとテスト失敗
B. Test Hacking（手抜き実装）と完成度
C. コードスタイルとフォーマット

**答え**: **B（Test Hacking（手抜き実装）と完成度）**
- AI Self Reviewは「テストを通すだけの仮実装」を検出
- 完成度0-100点評価で100点になるまで繰り返す
...
### ハンズオン全体の流れ
- 前提
  - 02で作成したGitHub Issues（Issue #4「todo_service.create()実装」を実装）
- 目的
  - STEP 4-5の全フローを実践
  - カスタムコマンド、TDD実装、8つの質問と指示を一気通貫で体験
- 成果物
...
- カスタムコマンドの復習（01で学習）
  - .claude/commands/ に Markdown ファイルとして保存
  - Git管理でチーム共有
- TDD用カスタムコマンド作成
  - コマンド名: /tdd
  - 機能: AI Self Reviewまでを自動実行（Human Reviewは人間が行う）
- カスタムコマンドの内容
...
### 【ステップ2】TDD実装
- カスタムコマンドを使った実装
  - Claude Codeに `/tdd 4` と指示
  - AIがAI Self Reviewまでを自動実行
- AIの処理を観察
  - Plan → Red → Green → Refactor → System Review → AI Self Review
- Human Review
  - git diffで変更内容を確認
...
### TDDと3段階レビュー
- knowledges/02-test-driven-development-tdd.md - Test Driven Development
- knowledges/18-trust-but-verify.md - 3段階レビュー
- knowledges/09-repomix-ai-code-review.md - Repomix AI Code Review
- knowledges/01-given-when-then-bdd.md - GIVEN WHEN THEN - BDD

### MCP Servers
- knowledges/03-model-context-protocol-mcp.md - Model Context Protocol
- knowledges/04-context7-mcp-server.md - Context7 MCP Server

### 品質改善・AI問題
- knowledges/07-cyclomatic-complexity.md - Cyclomatic Complexity
