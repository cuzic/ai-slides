...
- **設計書作成時間 vs 実装時間短縮（経験則）**
  - 設計書作成: 1-2時間
  - 実装時間短縮: 数時間〜
  - 投資対効果は高いが、プロジェクトにより変動
- **関連する研究データ**
  - AIコード脆弱性45%発生（Veracode 2025）→ 明示的仕様で軽減可能
  - 明示的仕様で修正要求68%削減（Microsoft TiCoder研究）
  - **仕様を明確にするほどAI出力の品質が向上**
- **品質向上の理由**
  - AIがコンテキストを正確に理解
  - エッジケース対応が漏れにくい
  - 複雑なロジックの実装精度向上
...
- **目的: 1つのサブタスク内でcompact（情報切り捨て）を発生させない**
  ...
  - **10分は経験則**：実務での試行錯誤から導いた目安
  - タスクの複雑さに応じて5-15分に調整可能
- **1つのタスクは10分で完了できるサイズに分割**
  - 30分以上なら暴走・手抜き・忘れっぽさが発生しやすい
- **サブタスク完了後は`/clear`を実行**
